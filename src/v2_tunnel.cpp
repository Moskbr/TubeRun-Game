...
std::vector<glm::vec3> generateControlPoints(int segmentCount, float segmentLength, float maxOffset, unsigned int seed)
{
    ...
    cp.reserve(segmentCount + 4);

    cp.push_back(glm::vec3(0.0f, 0.0f, -segmentLength));
    cp.push_back(glm::vec3(0.0f, 0.0f, 0.0f));
    cp.push_back(glm::vec3(0.0f, 0.0f, segmentLength));

    glm::vec3 current = cp.back();
                        ...
                        std::vector<glm::vec3>& N, std::vector<glm::vec3>& B)
{
    ...
    for (int i=1; i < n; i++)
    {
        ...
        Q[i] = glm::normalize(dq * Q[i-1]);
        N[i] = glm::normalize(dq * N[i-1]);
        B[i] = glm::normalize(dq * B[i-1]);
    }
}
